    ...
    function carrinho(){
          if (count($_SESSION['carrinho']) == 0) {
           ...
           foreach ($_SESSION['carrinho'] as $id => $qtd) {
               ...
               $sqlCarrinho = $this->con->conectar()->prepare("SELECT * FROM produtos AS p CROSS JOIN taxas WHERE p.indice_taxa = taxas.tipo AND  id_codigo = '$id'"); 
               $sqlCarrinho->execute();
             
               $resAssoc = $sqlCarrinho->fetch(PDO::FETCH_ASSOC);

             $imposto      = $resAssoc['imposto'];
             $nome         = $resAssoc ['nome'];
             $precoLiquido = $resAssoc ['preco_liquido'];
             ...
             $totalImposto += $subImposto;
              ...
              echo '<td><a href="index.php?pg=views/carrinho-compra&acao=del&id='.$id.'"><img src="views/img/ico-excluir.png"></td>';
              }
              echo '<tr>                   
